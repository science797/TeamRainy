package developmentteam.teamrainy.mod.modules;
...
import developmentteam.teamrainy.mod.Mod;
import developmentteam.teamrainy.mod.modules.impl.client.*;
import developmentteam.teamrainy.mod.modules.settings.Setting;
...
public abstract class Module extends Mod {
	...
	public Module(String name, String description, Category category) {
		...
		bindSetting = new BindSetting("Key", isGui() ? GLFW.GLFW_KEY_UP : -1);
	...
	private boolean listHide() {
		return this instanceof Colors || this instanceof BaritoneModule || this instanceof AntiCheat || this instanceof HackSetting || this instanceof ServerApply || this instanceof HUD || this instanceof ModuleList;
	}
	...
	public void enable() {
		...
		if (!nullCheck() && drawnSetting.getValue() && HackSetting.INSTANCE.toggle.getValue()) {
			int id = HackSetting.INSTANCE.onlyOne.getValue() ? -1 : hashCode();
			switch (HackSetting.INSTANCE.messageStyle.getValue()) {
				...
				case Moon -> CommandManager.sendChatMessageWidthIdNoSync("§f[§b" + HackSetting.INSTANCE.hackName.getValue() + "§f] [" + "§3" + getDisplayName() + "§f]" + " §7toggled §aon", id);
	...
	public void disable() {
		...
		if (!nullCheck() && drawnSetting.getValue() && HackSetting.INSTANCE.toggle.getValue()) {
			...
			switch (HackSetting.INSTANCE.messageStyle.getValue()) {
				...
				case Moon -> CommandManager.sendChatMessageWidthIdNoSync("§f[§b" + HackSetting.INSTANCE.hackName.getValue() + "§f] [" + "§3" + getDisplayName() + "§f]" + " §7toggled §coff", id);
			}
		}
		this.state = false;
