...
public class Ambience extends Module {
    ...
    public final BooleanSetting customTeamRainy =
            add(new BooleanSetting("CustomTeamRainy", false).setParent().injectTask(() -> {
                if (!nullCheck()) {
                    mc.worldRenderer.reload();
                }
            }));
    public final SliderSetting teamrainy =
            add(new SliderSetting("TeamRainy", 15, 0, 15, customTeamRainy::isOpen).injectTask(() -> {
                if (!nullCheck() && customTeamRainy.getValue()) {
                    mc.worldRenderer.reload();
                }
            }));
    public Ambience() {
