...
public class AutoTrap
    ...
    private void trapTarget(PlayerEntity target) {
        if (onlyHole.getValue() && !TeamRainy.HOLE.isHole(EntityUtil.getEntityPos(target))) return;
    ...
    private void doTrap(BlockPos pos) {
        ...
        if (antiStep.getValue() && (TeamRainy.BREAK.isMining(pos.up(2)) || !onlyBreak.getValue())) {
        ...
        if (chestUp.getValue()) {
            for (Direction i : Direction.values()) {
                ...
                if (!onlyBreaking.getValue() || TeamRainy.BREAK.isMining(pos.up(2))) {
                    tryPlaceObsidian(offsetPos);
    ...
    private void tryPlaceBlock(BlockPos pos, boolean anchor, boolean sand, boolean web) {
        ...
        if (TeamRainy.BREAK.isMining(pos)) return;
