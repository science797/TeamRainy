...
import developmentteam.teamrainy.api.utils.math.AnimateUtil;
...
import static developmentteam.teamrainy.api.utils.world.BlockUtil.*;


public class RainyAnchor extends Module {
	public static RainyAnchor INSTANCE;
	...
	public RainyAnchor() {
		super("RainyAnchor", Category.Combat);
		setChinese("自动重生锚");
		INSTANCE = this;
		TeamRainy.EVENT_BUS.subscribe(new AnchorRender());
	}
	...
	public void onUpdate() {
		...
		if (currentPos != null) {
			...
			boolean shouldSpam = this.spam.getValue() && (!mineSpam.getValue() || TeamRainy.BREAK.isMining(currentPos, false));
			if (shouldSpam) {
				...
				if (spamPlace.getValue() && inSpam.getValue()) {
					if (yawStep.getValue() && checkFov.getValue()) {
						...
						if (TeamRainy.ROTATION.inFov(directionVec, fov.getValueFloat())) {
					...
					for (PlayerAndPredict pap : list) {
						...
						if (getBlock(pos) != Blocks.RESPAWN_ANCHOR) {
							...
							if (damage >= placeDamage) {
								if (RainyCrystal.crystalPos == null || RainyCrystal.INSTANCE.isOff() || RainyCrystal.INSTANCE.lastDamage < damage) {
									double selfDamage;
									if ((selfDamage = getAnchorDamage(pos, selfPredict.player, selfPredict.predict)) > maxSelfDamage.getValue() || noSuicide.getValue() && selfDamage > mc.player.getHealth() + mc.player.getAbsorptionAmount()) {
										continue;
									}
	...
	public void onAssist() {
		...
		assistTimer.reset();
		double bestDamage;
		ArrayList<RainyAnchor.PlayerAndPredict> list = new ArrayList<>();
		for (PlayerEntity player : CombatUtil.getEnemies(assistRange.getValue())) {
			list.add(new RainyAnchor.PlayerAndPredict(player));
		}

		bestDamage = assistDamage.getValue();
		for (RainyAnchor.PlayerAndPredict pap : list) {
			BlockPos pos = EntityUtil.getEntityPos(pap.player, true).up(2);
	...
	public BlockPos getHelper(BlockPos pos) {
		for (Direction i : Direction.values()) {
			if (checkMine.getValue() && TeamRainy.BREAK.isMining(pos.offset(i))) continue;
