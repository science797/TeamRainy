...
import developmentteam.teamrainy.api.utils.math.*;
...
import developmentteam.teamrainy.mod.modules.impl.render.ExplosionSpawn;
...
import static developmentteam.teamrainy.api.utils.world.BlockUtil.getBlock;
import static developmentteam.teamrainy.api.utils.world.BlockUtil.hasCrystal;



public class RainyCrystal extends Module {
    public static RainyCrystal INSTANCE;
    ...
    // @f67m6m8:接下来请欣赏这最棒的算法
    public RainyCrystal() {
        super("RainyCrystal", Category.Combat);
        setChinese("自动水晶");
        INSTANCE = this;
        TeamRainy.EVENT_BUS.subscribe(new CrystalRender());
    }
    ...
    private boolean shouldReturn() {
        ...
        if (preferAnchor.getValue() && RainyAnchor.INSTANCE.currentPos != null) {
            lastBreakTimer.reset();
            return true;
        }
        return false;
    ...
    public boolean canPlaceCrystal(BlockPos pos, boolean ignoreCrystal, boolean ignoreItem) {
        ...
        return (getBlock(obsPos) == Blocks.BEDROCK || getBlock(obsPos) == Blocks.OBSIDIAN)
                ...
                && (!HackSetting.INSTANCE.lowVersion.getValue() || mc.world.isAir(boost2));
    }
    ...
    private void doBreak(BlockPos pos) {
        ...
        for (EndCrystalEntity entity : BlockUtil.getEndCrystals(new Box(pos.getX(), pos.getY(), pos.getZ(), pos.getX() + 1, pos.getY() + 2, pos.getZ() + 1))) {
            ...
            if (crystalPos != null && displayTarget != null && lastDamage >= getDamage(displayTarget) && afterBreak.getValue()) {
                if (!yawStep.getValue() || !checkFov.getValue() || TeamRainy.ROTATION.inFov(entity.getPos(), fov.getValueFloat())) {
                    doPlace(crystalPos);
                }
            }
