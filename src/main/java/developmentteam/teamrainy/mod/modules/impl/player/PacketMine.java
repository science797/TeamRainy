...
import developmentteam.teamrainy.api.events.impl.ClickBlockEvent;
...
import developmentteam.teamrainy.mod.modules.impl.combat.AutoCev;
...
public class PacketMine extends Module {
	...
	public void onUpdate() {
		...
		if (breakPos.equals(RainyAnchor.INSTANCE.currentPos)) return;
		...
		if (waitPlace.getValue()) {
			for (Direction i : Direction.values()) {
				if (breakPos.offset(i).equals(RainyCrystal.crystalPos)) {
					if (RainyCrystal.INSTANCE.canPlaceCrystal(RainyCrystal.crystalPos, false, false)) {
						return;
					}
					break;
				}
	...
	public static boolean canPlaceCrystal(BlockPos pos, boolean ignoreItem) {
		...
		return (BlockUtil.getBlock(obsPos) == Blocks.BEDROCK || BlockUtil.getBlock(obsPos) == Blocks.OBSIDIAN)
				...
				&& (!HackSetting.INSTANCE.lowVersion.getValue() || mc.world.isAir(boost.up()));
	}
	...
	public float getDigSpeed(BlockState state, ItemStack itemStack) {
		...
		boolean inWeb = TeamRainy.PLAYER.isInWeb(mc.player) && mc.world.getBlockState(breakPos).getBlock() == Blocks.COBWEB;
