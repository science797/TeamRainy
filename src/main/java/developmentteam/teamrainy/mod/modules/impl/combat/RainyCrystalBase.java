...
public class RainyCrystalBase extends Module {
    public static RainyCrystalBase INSTANCE;
    ...
    public RainyCrystalBase() {
        super("RainyCrystalBase", Category.Combat);
        setChinese("水晶自动放黑曜石");
        INSTANCE = this;
    }
    ...
    private void update() {
        ...
        if (breakOnlyHasCrystal.getValue() && !mc.player.getMainHandStack().getItem().equals(Items.END_CRYSTAL) && !mc.player.getOffHandStack().getItem().equals(Items.END_CRYSTAL) && !RainyCrystal.INSTANCE.findCrystal()) {
            tempPos = null;
            return;
        }
        delayTimer.reset();
    ...
    private void doPlace(BlockPos pos) {
        ...
        if (detectMining.getValue() && TeamRainy.BREAK.isMining(pos)) return;
        int block = getBlock();
        if (block == -1) return;
