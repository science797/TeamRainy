...
import static developmentteam.teamrainy.api.utils.world.BlockUtil.canReplace;
...
public class Burrow extends Module {
    ...
    public void onUpdate() {
        if (TeamRainy.PLAYER.isInWeb(mc.player)) {
            webTimer.reset();
            return;
        }
        ...
        if (this.rotate.getValue() == RotateMode.Bypass) {
            TeamRainy.ROTATION.snapAt(TeamRainy.ROTATION.rotationYaw, 90);
        }
    ...
    private void gotoPos(BlockPos offPos) {
        if (rotate.getValue() == RotateMode.None) {
            ...
            mc.getNetworkHandler().sendPacket(new PlayerMoveC2SPacket.Full(offPos.getX() + 0.5, mc.player.getY() + 0.1, offPos.getZ() + 0.5, TeamRainy.ROTATION.rotationYaw, 90, false));
        }
    }
    ...
    private boolean canPlace(BlockPos pos) {
        ...
        if (detectMine.getValue() && TeamRainy.BREAK.isMining(pos)) {
            return false;
        }
