...
import developmentteam.teamrainy.mod.modules.impl.movement.MoveFix;
...
public class RotationManager implements Wrapper {
    ...
    public void snapBack() {
        mc.getNetworkHandler().sendPacket(new PlayerMoveC2SPacket.Full(mc.player.getX(), mc.player.getY(), mc.player.getZ(), TeamRainy.ROTATION.rotationYaw, TeamRainy.ROTATION.rotationPitch, mc.player.isOnGround()));
    }
    ...
    public void snapAt(Vec3d directionVec) {
        ...
        if (AntiCheat.INSTANCE.noSpamRotation.getValue()) {
            if (MathHelper.angleBetween(angle[0], TeamRainy.ROTATION.lastYaw) < AntiCheat.INSTANCE.fov.getValueFloat() && Math.abs(angle[1] - TeamRainy.ROTATION.lastPitch) < AntiCheat.INSTANCE.fov.getValueFloat()) {
                return;
            }
        }
    ...
    public void update(MovementPacketsEvent event) {
        if (MoveFix.INSTANCE.isOn() && !BaritoneModule.isActive()) {
            ...
            TeamRainy.EVENT_BUS.post(event1);
    ...
    private void updateNext() {
        ...
        TeamRainy.EVENT_BUS.post(rotateEvent);
    ...
    public void onLastRotation(RotateEvent event) {
        ...
        TeamRainy.EVENT_BUS.post(lookAtEvent);
