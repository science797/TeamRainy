...
import developmentteam.teamrainy.asm.accessors.IChatHud;
...
import developmentteam.teamrainy.api.interfaces.IChatHudLine;
...
public abstract class MixinChatHud implements IChatHudHook {
    ...
    public int chatLinesSize(List<ChatHudLine.Visible> list) {
        return HackSetting.INSTANCE.isOn() && HackSetting.INSTANCE.infiniteChat.getValue() ? -2147483647 : list.size();
    }
    ...
    private int drawStringWithShadow(DrawContext drawContext, TextRenderer textRenderer, OrderedText text, int x, int y, int color) {
        if (HackSetting.chatMessage.containsKey(text) && HackSetting.chatMessage.get(text).getString().startsWith(CommandManager.syncCode)) {
            if (HackSetting.INSTANCE.pulse.booleanValue) {
                return TextUtil.drawStringPulse(drawContext, text, x, y, ColorUtil.injectAlpha(HackSetting.INSTANCE.color.getValue(), ((color >> 24) & 0xff)),ColorUtil.injectAlpha(HackSetting.INSTANCE.pulse.getValue(), ((color >> 24) & 0xff)), HackSetting.INSTANCE.pulseSpeed.getValue(), HackSetting.INSTANCE.pulseCounter.getValueInt());
            }
            return drawContext.drawTextWithShadow(textRenderer, text, x, y, ColorUtil.injectAlpha(HackSetting.INSTANCE.color.getValue(), ((color >> 24) & 0xff)).getRGB());
        }
    ...
    private int get(int i) {
        ...
        if (last != null && !map.containsKey(last)) {
            map.put(last, new FadeUtils(HackSetting.INSTANCE.animateTime.getValueInt()));
        }
        return i;
    }
    ...
    private void translate(DrawContext context, int currentTick, int mouseX, int mouseY, CallbackInfo ci) {
        if (map.containsKey(last)) {
            context.getMatrices().translate(HackSetting.INSTANCE.animateOffset.getValue() * (1 - map.get(last).ease(HackSetting.INSTANCE.ease.getValue())), 0.0, 0.0f);
        }
    }
    ...
    private float applyYOffset(float y) {
        fadeTime = HackSetting.INSTANCE.fadeTime.getValueFloat();
        if (HackSetting.INSTANCE.yAnim.getValue()) {
            calculateYOffset();
    ...
    private double modifyOpacity(double originalOpacity) {
        ...
        if (HackSetting.INSTANCE.fade.getValue()) {
            try {
                double quad = messageTimestamps.get(chatLineIndex).ease(HackSetting.INSTANCE.ease.getValue());
    ...
    private MessageIndicator removeMessageIndicator(MessageIndicator messageIndicator) {
        if (HackSetting.INSTANCE.hideIndicator.getValue()) {
            return null;
        }
